...
class SegResNet_Detection_Model(nn.Module):
    def __init__(self, config):
        ...
        self.backbone = SegResNetBackbone(
            ...
            use_conv_final=config.use_conv_final,
            upsample_mode=config.upsample_mode
        )
        
        self.confidence_threshold = config.confidence_threshold
        self.class_weighting = torch.tensor(config.class_weights) if hasattr(config, 'class_weights') else None

    def forward(self, volume, labels=None):
        _, feature_maps = self.backbone(volume)
        
        if self.training and labels is not None:
            # Check if we have pre-computed target blocks (from Mixup)
            if 'target_blocks' in labels:
                # Use pre-computed mixed target blocks
                target_block = labels['target_blocks']
                target_shape = target_block.shape[2:]  # [D, H, W]
            else:
                # Original coordinate-based target creation
                target_shape = feature_maps[-1].shape[2:]
                target_block = torch.zeros([feature_maps[-1].shape[0], 2, *target_shape], 
                                         dtype=torch.float, device=volume.device)
                target_block[:, 0, :, :, :] = 1.0  # Background
                
                for batch_idx, coord in enumerate(labels['coords']):  
                    if coord is not None and coord[0] != -1:
                        z, y, x = coord
                        target_block[batch_idx, 0, z, y, x] = 0.0
                        target_block[batch_idx, 1, z, y, x] = 1.0
            
            criterion = DenseCrossEntropy(
                class_weights=self.class_weighting.to(volume.device) if self.class_weighting is not None else None
            )
            losses = []
            ...
            for i, feature_map in enumerate(feature_maps[-3:-1]):
                ...
                loss_val, _ = criterion(upsampled, target_block)
                losses.append(loss_val * (0.5 ** i))
            ...
            for batch_idx in range(motor_probs.shape[0]):
                ...
                if max_prob > self.confidence_threshold:
                    max_idx = motor_probs[batch_idx].argmax()
...
class DenseCrossEntropy(nn.Module):
    ...
    def forward(self, x, target):
        ...
        if self.class_weights is not None:
            loss = (class_losses * self.class_weights.to(class_losses.device)).sum()
        else:
...
def create_target_blocks(volume, labels, model):
    """Convert coordinate labels to dense target blocks for Mixup"""
    # Get the expected output shape from model
    with torch.no_grad():
        _, feature_maps = model.backbone(volume[:1])  # Use single sample to get shape
        target_shape = feature_maps[-1].shape[2:]
    
    target_block = torch.zeros([volume.shape[0], 2, *target_shape], 
                             dtype=torch.float, device=volume.device)
    target_block[:, 0, :, :, :] = 1.0  # Background
